...
// #endif 
	...
	case PLAY  : {
			...
			unsigned int remain = decoder->max_frames_in_buffer - decoder->frames_in_buffer;
			unsigned int pos_update = 0;
			if ( remain >= decoder->block_size )
			{
				decoder->decode( direction, f_position, 
				                 decoder->block_size, 
						 &decoder->m_readBuffer[ decoder->frames_in_buffer ]);
				pos_update = decoder->block_size;
			}
			else
			{
				decoder->decode( direction, f_position, 
			...
			// calculating position update offset 
			if ( direction == BACKWARD )
				pos_update =  -(pos_update);
			// update frame position:
			pthread_mutex_lock(&decoder->decoder_mutex);
			decoder->m_frame_position += pos_update;
			pthread_mutex_unlock(&decoder->decoder_mutex);
...
void DecoderBase::performDsp()
{
  ...
  // Our decoder unit has no other way to make work the
  // whole chain than calling run() to the out DspUnit linked.
  m_outDspUnits[0]->run();
